...
class SignUpThirdVC: BasicVC, UITextFieldDelegate{
    ...
    let tabGesture: UITapGestureRecognizer = UITapGestureRecognizer(target: SignUpThirdVC.self, action: nil)
    ...
    fileprivate func userNameChecked(inputUserName: String) {
        AlamofireManager.shared.getExistCheck(errorType: "username", username: inputUserName, email: nil, completion: {
            ...
            switch result {
            case .success(_):
                print("SignUpThirdVC - getExistCheck.success")
                // 사용가능 문구 띄우기
            ...
            case .failure(let error):
                print("SignUpThirdVC - getExistCheck.failure / error: \(error)")
                // 중복사용 문구 띄우기
